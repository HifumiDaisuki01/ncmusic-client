...
public class TracksScreen extends Screen {
	...
	public TracksScreen(Screen parent, NcPlaylist playlist) {
		super(new LiteralText(playlist.name));
		this.parent = parent;
	...
	protected void init() {
		int cx = this.width / 2;
		this.addButton(new ButtonWidget(cx - 206, this.height - 30, 100, 20,
				new LiteralText("顺序播放"), b -> playAll(PlayMode.SEQUENCE)));
		this.addButton(new ButtonWidget(cx - 100, this.height - 30, 100, 20,
				new LiteralText("随机播放"), b -> playAll(PlayMode.SHUFFLE)));
		this.addButton(new ButtonWidget(cx + 106, this.height - 30, 100, 20,
	...
	public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
		...
		for (int i = 0; i < rows; i++) {
			...
			if (hovered) fill(matrices, cx - 200, y, cx + 200, y + ROW_H, 0x33FFFFFF);
			String row = (idx + 1) + ". " + s.name;
			this.textRenderer.drawWithShadow(matrices, trim(row, 240), cx - 194, y + 4, 0xFFFFFFFF);
			this.textRenderer.drawWithShadow(matrices, trim(s.artists, 140), cx + 60, y + 4, 0xFF9A9A9A);
		}
	}
