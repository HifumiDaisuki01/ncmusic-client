...
public class BrowserLoginScreen extends Screen {
	...
	public BrowserLoginScreen(Screen parent) {
		super(new LiteralText("浏览器登录网易云"));
		this.parent = parent;
	}
	...
	protected void init() {
		...
		this.addButton(new ButtonWidget(cx - 150, y, 300, 20,
				new LiteralText("打开浏览器并登录"), b -> startLogin()));
		this.addButton(new ButtonWidget(cx - 50, y + 30, 100, 20,
	...
	private void startLogin() {
		...
		executor.submit(() -> {
			try {
				...
				if (exe == null) {
					status = "未找到 Chrome/Edge。请改用 Cookie 文件方式登录（见 README）";
					running = false;
					return;
				}
	...
	public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
		...
		drawCenteredText(matrices, this.textRenderer, title, cx, this.height / 2 - 60, 0xFFEC4141);
		String[] lines = {
		...
		drawCenteredText(matrices, this.textRenderer, s, cx, this.height / 2 + 4, 0xFFFFFFFF);
	}
