...
public class NcMusicClient implements ClientModInitializer, MusicPlayer.Listener {
	...
	public void onInitializeClient() {
		...
		// 按键：M 打开控制面板
		panelKey = KeyMappingHelper.registerKeyMapping(new KeyMapping(
				"key.ncmusic.panel", GLFW.GLFW_KEY_M,
				new KeyMapping.Category(Identifier.fromNamespaceAndPath(MOD_ID, "music"))));

		ClientTickEvents.END_CLIENT_TICK.register(mc -> {
			// 扫码登录轮询（不依赖界面 tick，双保险）
			if (mc.gui.screen() instanceof QrLoginScreen qr) qr.pollIfDue();
			while (panelKey.consumeClick()) {
				if (mc.gui.screen() instanceof MusicControlScreen) {
					mc.gui.setScreen(null);
				} else if (mc.gui.screen() == null) {
					mc.gui.setScreen(new MusicControlScreen(null));
				}
			}
		});
	...
	public void onError(String message) {
		...
		mc.execute(() -> {
			if (mc.player != null) {
				mc.player.sendSystemMessage(Component.literal("[NC音乐] " + message));
			}
		});
	}
}
