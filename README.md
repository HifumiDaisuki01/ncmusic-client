...
| 游戏版本 | 需要 Java | Fabric API | 对应 jar |
| --- | --- | --- | --- |
| **26.2**（主版本） | 25+ | 0.159.0+26.2 | `ncmusic-client-1.0.0.jar` |
| **1.21.11** | 21+ | 0.141.6+1.21.11 | `ncmusic-client-12111-1.0.0.jar` |
| **1.20.1** | 17+ | 0.92.12+1.20.1 | `ncmusic-client-1201-1.0.0.jar` |
| **1.16.5**（无 /ncm 命令，全功能并入 M 键面板） | 11+ | 0.42.0+1.16 | `ncmusic-client-1165-1.0.0.jar` |

- 全部 jar 在右侧 **Releases** 下载（每个版本一个 jar）。
- 各版本移植版源码在 [`mc-ports/`](mc-ports/) 目录（`1165` / `1201` / `12111`），构建方式见各目录 README。
- 功能四版本一致：浏览器一键登录/扫码/手机号/Cookie 登录、搜索、完整歌单、顺序/随机播放、HUD 封面/歌词/进度/列表。
...
2. 从 GitHub Release 下载对应版本的 jar，与对应版本的 **Fabric API**
   一起放进 `mods/` 目录。
