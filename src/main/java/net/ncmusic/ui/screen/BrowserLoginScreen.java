...
public class BrowserLoginScreen extends Screen {
			...
			try {
				...
				if (cookie != null && NetEaseApi.INSTANCE.loginByCookie(cookie)) {
					...
					executor.submit(() -> {
						...
						mc.execute(() -> mc.gui.setScreen(new MusicControlScreen(
	...
	public void extractRenderState(GuiGraphicsExtractor gfx, int mouseX, int mouseY, float delta) {
		...
		gfx.text(this.font, title, cx - this.font.width(title) / 2, this.height / 2 - 60, 0xFFEC4141, false);
		String[] lines = {
		...
		for (String l : lines) {
			gfx.text(this.font, l, cx - this.font.width(l) / 2, y, 0xFF9A9A9A, false);
			y += 12;
		}
		String s = running ? status : "等待开始…";
		gfx.text(this.font, s, cx - this.font.width(s) / 2, this.height / 2 + 4, 0xFFFFFFFF, false);
	}
