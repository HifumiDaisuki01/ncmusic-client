...
public final class NcmCommands {
	...
	public static void register() {
		ClientCommandRegistrationCallback.EVENT.register((dispatcher, registryAccess) -> dispatcher.register(
				ClientCommandManager.literal("ncm")
						.executes(ctx -> {
						...
						.then(ClientCommandManager.literal("browser").executes(ctx -> {
							open(new BrowserLoginScreen(null));
							return 1;
						}))
						.then(ClientCommandManager.literal("loginphone")
						...
						.then(ClientCommandManager.literal("login-cookie")
						...
						.then(ClientCommandManager.literal("cookie-file").executes(ctx -> {
						...
						.then(ClientCommandManager.literal("playlist").executes(ctx -> {
							open(new PlaylistsScreen(null));
							return 1;
						}))
						.then(ClientCommandManager.literal("play").executes(ctx -> {
						...
						.then(ClientCommandManager.literal("hud")
								...
								.then(ClientCommandManager.literal("corner").executes(ctx -> {
									NcmConfig cfg = NcmConfig.get();
	...
	private static void feedback(String msg) {
		...
		Minecraft mc = Minecraft.getInstance();
		if (mc.player != null) mc.player.displayClientMessage(Component.literal("[NC音乐] " + msg), false);
	}
}
