...
public class CoverManager {
	...
	private final Map<Long, Identifier> cache = new LinkedHashMap<>(16, 0.75f, true);
	...
	public Identifier get(NcSong song) {
		if (song == null || song.coverUrl.isEmpty()) return null;
		synchronized (cache) {
			Identifier id = cache.get(song.id);
			if (id != null) {
				touch(id);
				return id;
			}
	...
	private void touch(Identifier keep) {
		while (cache.size() > MAX_CACHE) {
			Long eldest = cache.keySet().iterator().next();
			Identifier evict = cache.remove(eldest);
	...
	private void register(long songId, NativeImage image) {
		synchronized (cache) {
			...
			Identifier id = Identifier.fromNamespaceAndPath("ncmusic", "cover_" + songId);
			DynamicTexture tex = new DynamicTexture(() -> "ncmusic cover " + songId, image);
			tex.upload();
