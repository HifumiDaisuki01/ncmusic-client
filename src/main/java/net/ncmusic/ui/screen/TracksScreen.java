...
public class TracksScreen extends Screen {
	...
	public void extractRenderState(GuiGraphicsExtractor gfx, int mouseX, int mouseY, float delta) {
		...
		for (int i = 0; i < rows; i++) {
			...
			if (hovered) gfx.fill(cx - 200, y, cx + 200, y + ROW_H, 0x33FFFFFF);
			String row = (idx + 1) + ". " + s.name;
			gfx.text(this.font, trim(row, 240), cx - 194, y + 4, 0xFFFFFFFF, false);
			gfx.text(this.font, trim(s.artists, 140), cx + 60, y + 4, 0xFF9A9A9A, false);
		}
	}
