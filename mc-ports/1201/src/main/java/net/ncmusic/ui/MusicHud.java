 ...
 */
public class MusicHud implements HudRenderCallback {
	...
	public void onHudRender(GuiGraphics g, float tickDelta) {
		...
		// 左侧强调条
		g.fill(x, y, x + 2, y + panelH, C_ACCENT);

		int cx = x + PAD + 2;
		int cy = y + PAD;

		if (noticeVisible) {
			g.drawString(font, trim(font, notice, PANEL_W - PAD * 2 - 2), cx, cy, 0xFFFFA0A0, false);
			cy += noticeH;
		}

		if (!hasSong) {
			g.drawString(font, "网易云音乐 · 按 M 打开面板", cx, cy, C_SUB, false);
			return;
		}
		...
		if (infoH > 0) {
			int textX = cx;
			if (cfg.showCover) {
				ResourceLocation cover = CoverManager.INSTANCE.get(song);
				if (cover != null) {
					g.blit(cover, cx, cy, 0, 0, 36, 36, 36, 36);
				} else {
					g.fill(cx, cy, cx + 36, cy + 36, 0xFF222222);
				}
				textX += 42;
			}
			if (cfg.showSongInfo) {
				int textW = x + PANEL_W - PAD - textX;
				g.drawString(font, trim(font, song.name, textW), textX, cy + 4, C_TITLE, false);
				String sub = song.artists.isEmpty() ? song.album : song.artists;
				g.drawString(font, trim(font, sub, textW), textX, cy + 16, C_SUB, false);
				...
				g.drawString(font, stateText, textX, cy + 28, C_ACCENT, false);
			}
			cy += infoH;
		}
		...
		if (progressH > 0) {
			...
			g.fill(cx, cy + 2, cx + barW, cy + 5, C_BAR_BG);
			g.fill(cx, cy + 2, cx + (int) (barW * ratio), cy + 5, C_ACCENT);
			g.drawString(font, fmtTime(pos) + " / " + fmtTime(dur), cx, cy + 8, C_SUB, false);
			cy += progressH;
		}
		...
		if (lyricH > 0) {
			if (lyric != null && !lyric.isEmpty()) {
				...
				g.drawString(font, trim(font, curText, textW), cx, cy, C_LYRIC, false);
				g.drawString(font, trim(font, nextText, textW), cx, cy + 11, C_LYRIC_NEXT, false);
			} else {
				g.drawString(font, "暂无歌词", cx, cy, C_LYRIC_NEXT, false);
			}
			cy += lyricH;
		}
		...
		if (listH > 0) {
			...
			for (int i = 0; i < size; i++) {
				...
				g.drawString(font, prefix + trim(font, row, textW - font.width(prefix)), cx, cy, color, false);
				cy += 11;
			}
		}
	}
